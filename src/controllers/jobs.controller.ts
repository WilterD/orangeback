...
export const addJob = async (
  ...
  try {
    ...
    const response = await pool.query({
      text: `SELECT * FROM jobs WHERE job_id = ${insertedId}`,
    });
...
export const updateJob = async (
  ...
  try {
    ...
    return successResponse(res, STATUS.OK, "Trabajo modificado éxitosamente");
  } catch (error: unknown) {
    console.log(error);
    return handleControllerError(error, res);
  }
...
export const deleteJob = async (
  ...
  try {
    ...
    return successResponse(res, STATUS.OK, "Trabajo eliminado éxitosamente");
  } catch (error: unknown) {
    return handleControllerError(error, res);
  }
};
