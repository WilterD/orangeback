...
export const getDiscountsDataFromRequestBody = (req: Request): any[] => {
  const { percentage, servicesMin, servicesMax, agencyRif } = req.body;
  const newDiscount = [percentage, servicesMin, servicesMax, agencyRif];
  return newDiscount;
};
  ...
  try {
    ...
    const insertar = await pool.query({
      text: `INSERT INTO discounts (
          percentage, services_min, services_max, 
          agency_rif
        ) 
        VALUES 
          ($1, $2, $3, $4) RETURNING discount_id
        `,
      values: newDiscount,
    });
