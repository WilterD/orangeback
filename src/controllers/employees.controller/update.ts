...
const getEmployeesUpdateDataFromRequestBody = (
  req: Request
): [any[], number[]] => {
  const { name, phone, address, salary, agencyRif, jobId, servicesIds } =
    req.body

  const updateEmployee = [name, phone, address, salary, agencyRif, jobId]
